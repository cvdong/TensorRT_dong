...
bool build_model(){
    ...
    FILE* f = fopen("yolov5s.engine", "wb");
...
void inference(){
    ...
    // 加载engine
    auto engine_data = load_file("yolov5s.engine");
    ...
    if(engine->getNbBindings() != 2){
        printf("onnx导出有问题，必须是1个输入和1个输出，你这明显有：%d个输出.\n", engine->getNbBindings() - 1);
        return;
    }
    ...
    */
    ...
    // pre-process
    // 利用仿射变换实现yolov5 pre,这里主要要理解仿射变换相关原理
    // 方便后续cuda实现
    ...
    cv::invertAffineTransform(, m2x3_d2i);  // 计算一个逆仿射变换
  ...
  */
  // host->device
    ...
    // post-process
    // decode box：
    // 从不同尺度下的预测狂还原到原输入图上(包括:预测框，类被概率，置信度）
